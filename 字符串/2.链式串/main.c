...
int main() {
	...
	printf("S+S1连接后串S2"); printStr(S2);
	printf("\n\n****************串拷贝*****************\n");
	StrCopy(S, S2);//将S2拷贝到S中
	printf("S2拷贝到S中,拷贝后串S");
	printStr(S);
	printf("\n\n****************插入串*****************\n");
	StrInsert(S, 5, S1);//在串S第一个位置后面插入S1
	printf("在串S第一个位置后面插入S1,插入后串S");
	printStr(S);
	printf("\n\n****************截取子串*****************\n");
	SubString(S,T, 1, 4);//截取串S第一个位置开始的四位
	printf("截取S串1到4的子串");
	printStr(T);
	return 0;
}
